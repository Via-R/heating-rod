...
def print_end_conditions(
    ...
    print(
        f"Rod division length: {estimated_division_length:.2f}, divisions: {int(divisions * 2)}"
    )
    print("*" * 36, end="\n\n")
...
def rod_temperature_demo(
    ...
    ax.plot(
        x_values,
        [desired_temperature] * heaters_amount * 2 * rod_part_points_amount,
        color="grey",
    )
    frame_text = ax.text(
        0.7,
...
def rod_division_demo(
    max_temperature, rod_length, thermal_conductivity, desired_min_temperature, max_time
):
